...
- If **no-hook**: run agentic-setup manually before committing:
  ...
  2. Run: `agentic-setup refresh && git add AGENTIC_LEARNINGS.md CLAUDE.md .claude/ .cursor/ .cursorrules 2>/dev/null`
...
- **Hooks**: `.cursor/hooks.json` · `.claude/hooks/agentic-session-freshness.sh` · `.claude/hooks/agentic-check-sync.sh`
...
- Verify the configuration setup by executing `./gradlew --version`.
...
- Sync validation check automatically triggered before commits.